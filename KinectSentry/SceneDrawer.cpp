...
void drawFilledCircle(XnUserID player, XnSkeletonJoint eJoint1, XnSkeletonJoint eJoint2){
	...
	GLfloat twicePi = 2.0f * 3.4;

	glBegin(GL_TRIANGLE_FAN);
	for (i = 0; i < 4; i++)
	{
		
		glVertex2f(pt[0].X, pt[0].Y); // center of circle
		for (j = 0; j <= triangleAmount; j++)
		{
			glVertex2f(
				pt[0].X + (radius * cos(j *  twicePi / triangleAmount)),
